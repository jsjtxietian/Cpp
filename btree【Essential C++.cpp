...
insert_value(const valType &val)
{
	if(val==_val)
	{
		...
		(*BinaryTree<valType>::os()) 
			<<"BTnode::insert_value: increment count( " 
			<< val << ":" << _cnt << ")\n";
	...
	if(val < _val)
	{
		if(!_lchild)
		{
			...
			(*BinaryTree<valType>::os()) 
				<< "ok: BTnode::insert_value at left child( " 
				<< val << " )\n";
		}
		else
		{
	...
	else
	{
		if ( ! _rchild )
		{
			...
			(*BinaryTree<valType>::os()) 
				<< "ok: BTnode::insert_value at right child( " 
				<< val << " )\n";		 
		}
		else 
		{
...
class BinaryTree
{
	public:
		...
		void clear() { clear(_root); _root=0; }
...
insert(const elemType &elem)
{
	if(!_root)
	{
		(*BinaryTree<elemType>::os()) 
			<< "BinaryTree::insert: root( " << elem << " )\n";
